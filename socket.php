...
$port = 1985;
    ...
    if (($ref = socket_bind($sock, $address, $port)) < 0) {
        throw new Exception('socked_bind() failed: ' . socket_strerror(socket_last_error()) . "\n");
    } else {
        echo "OK \n";
    }

    echo 'Listen socket ...';
    if (($ref = socket_listen($sock, 5)) < 0) {
        throw new Exception('socked_listen() failed: ' . socket_strerror(socket_last_error()) . "\n");
    } else {
        echo "OK \n";
    }
